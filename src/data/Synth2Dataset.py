...
class Synth2Dataset(IDataset):
    ...
    def _make_cluster(self, config: ClusterConfig) -> np.array:
        cluster_coords = self._rng.multivariate_normal(config.mean, config.cov, config.size)
        if self._discrete_m:
            return np.hstack([cluster_coords, np.vstack([config.m]*config.size)])
        else:
            return np.hstack([cluster_coords, np.vstack(self._rng.normal(config.m, self._var_m, config.size))])
    ...
    def __init__(self):
        ...
        self._offset_factor = 100 # offset for in meters for distance of 1
        self._cluster_configs = [
            self.ClusterConfig(np.array([6,6,6]), np.identity(3)*0.5, 1000, 1),
            self.ClusterConfig(np.array([1,2,3]), np.identity(3)*1, 5000, 2),
            self.ClusterConfig(np.array([10,0,0]), self._to_spd(self._rng.random(9)), 200, 3),
            self.ClusterConfig(np.array([0,6,9]), self._to_spd(self._rng.normal(2, 0.5, 9)), 2000, 4),
            self.ClusterConfig(np.array([6,1,9]), np.identity(3)*np.array([1,2,3]), 700, 5)
        ]
